...
var templateDir = path.join(rootDir, 'templates') + path.sep;
var EOL = '\n';
var SUB_SILDE_PLACEHOLDER = '~~NODEPPT%THEO●WANG%NODEPPT%~~';
var M_SUB_SILDE_PLACEHOLDER = marked(SUB_SILDE_PLACEHOLDER);

//默模板,根据参数重写
var templateFile = path.join(rootDir, 'templates/simple') + path.sep + 'index.ejs';
...
var parser = function (string, callback, argvObj, queryObj, commonData, templateDir) {

    templateFile = path.join(rootDir, "templates" + path.sep + templateDir) + path.sep + 'index.ejs';
...
function parse(content, extendTags, note, sAttrs) {
    ...
    return note + '<section class="swiper-slide"' + sAttrs +'>' + content + '</section>';
}
